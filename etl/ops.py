...
import pandas as pd
from crud.sanitization import sanitize_cols
...
from .casting_pipeline import _filter_cast_kwargs, _get_schema_guided_dtypes, _load_and_sanitize_data, _extract_semantic_meta
...
class ETL:
    ...
    def create_load(
        ...
        **kwargs
        ...
        df_raw, df_cast, pk_cols, semantic_meta = _load_and_sanitize_data(self.ctx, source, table, pk, failure_threshold, **kwargs)
        df_final, ddl, meta = _prepare_ddl_and_sync(self.ctx, df_raw, df_cast, table, pk_cols, res_rc, source, fk, unique, autoincrement, varchar_sizes)
        ...
        return _load_and_validate(self.ctx, sm, table, df_final, 'insert', None, chunksize, add_missing_cols, failure_threshold, semantic_meta, trace_sql=trace_sql)
    ...
    def append(
        ...
        **kwargs
        ...
        return _load_and_validate(self.ctx, sm, table, df_final, 'insert', None, chunksize, add_missing_cols, failure_threshold, clean_meta, trace_sql=trace_sql)
    ...
    def upsert(
        ...
        **kwargs
        ...
        df_raw, df_cast, pk_cols, semantic_meta = _load_and_sanitize_data(self.ctx, source, table, res_pk, failure_threshold, **kwargs)
        df_final = sanitize_cols(df_cast, dialect=self.dialect) if res_rc else df_cast
        ...
        return _load_and_validate(self.ctx, sm, table, df_final, 'upsert', pk_cols, chunksize, add_missing_cols, failure_threshold, semantic_meta, trace_sql=trace_sql)
    ...
    def auto_etl(
        ...
        **kwargs
        ...
        try:
            df_raw, df_cast, pk_cols, semantic_meta = _load_and_sanitize_data(self.ctx, source, table, res_pk, kwargs.get('failure_threshold'), **kwargs)
            if pk_cols:
