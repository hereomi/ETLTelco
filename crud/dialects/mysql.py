...
def mysql_upsert(engine: Engine, data: DataLike, table: Union[str, sa.Table], constrain: List[str], chunk: int = 10_000, tolerance: int = 5, trace_sql: bool = False, strict: bool = True) -> Dict[str, Any]:
...
def mysql_insert(engine: Engine, data: DataLike, table: Union[str, sa.Table], chunk_size: int = 10_000, tolerance: int = 5, trace_sql: bool = False, strict: bool = True) -> Dict[str, Any]:
    rows = _normalize_data(data)
    if not rows:
        return {"total": 0, "success": 0, "failed": 0, "method": "none", "diagnostics": {"mode": "strict" if strict else "non_strict", "fallback_used": False, "row_errors": []}}
    stats = {"total": len(rows), "success": 0, "failed": 0, "chunks": [], "diagnostics": {"mode": "strict" if strict else "non_strict"}}
