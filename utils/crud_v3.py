...
def select_sql(table_name: str, where_conditions: List[Tuple[str, str, Any]], dialect: str = "postgresql", columns: Optional[List[str]] = None) -> SQLQuery:
    ...
    for i, (col, op, val) in enumerate(where_conditions):
        if op.upper() not in ALLOWED_OPERATORS: raise ValueError(f"Invalid operator: {op}")
        param_name = f"w_{i}"
    ...
    if where_parts: sql += f" WHERE {' AND '.join(where_parts)}"
    return SQLQuery(sql, params)
...
def update_sql(table_name: str, data: Dict[str, Any], where_conditions: List[Tuple[str, str, Any]], dialect: str = "postgresql") -> SQLQuery:
    ...
    if not data: raise ValueError("No data provided for update")
...
def lazy_update_sql(data: DataLike, table: str, where: List[Tuple[str, str, Any]], dialect: str = "postgresql", constrain: Optional[List[str]] = None, already_normalized: bool = False) -> SQLQuery:
    ...
    rows = data if already_normalized else _normalize_data(data)
    if not isinstance(rows, list) or not rows: raise ValueError("No data provided for update")
    ...
    if constrain:
        ...
        if not row: raise ValueError(f"No update columns match constrain: {constrain}")
    
    return update_sql(table, row, where, dialect)


if __name__ == "__main__":
    # Example usage - Pure SQL generation without engine
    data = [{"id": 1, "username": "alice", "city": "Paris"}]
    
    # Generate SQL for different dialects
    insert_queries = lazy_insert_sql(data, "users", "postgresql")
    for q in insert_queries:
        print(f"Insert SQL: {q.sql}")
        print(f"Params: {q.params}")
    
    upsert_queries = lazy_upsert_sql(data, "users", ["id"], "sqlite")
    for q in upsert_queries:
        print(f"Upsert SQL: {q.sql}")
        print(f"Params: {q.params}")
    
    update_query = lazy_update_sql([{"city": "Lyon"}], "users", [("id", "=", 1)], "oracle")
    print(f"Update SQL: {update_query.sql}")
    print(f"Params: {update_query.params}")
    
    select_query = lazy_select_sql([("id", "=", 1)], "users", "postgresql", ["username", "city"])
    print(f"Select SQL: {select_query.sql}")
    print(f"Params: {select_query.params}")