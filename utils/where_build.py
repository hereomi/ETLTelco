...
def _align_column_name(field: str, df_cols: set[str]) -> Tuple[str, bool]:
    ...
    Args:
        field: Field name to align
        df_cols: Set of DataFrame column names
    
    Returns:
        Tuple of (aligned_field_name, found_in_df)
    """
...
def _parse_where_item(item: Any, df_cols: set[str], allow_missing: bool = False) -> Optional[Dict[str, Any]]:
    ...
    Args:
        item: WHERE clause item (tuple, string, or dict)
        df_cols: Set of DataFrame column names
        allow_missing: If True, allow columns not in DataFrame (for literal values)
    
    Returns:
        Dictionary with field, operator, and value, or None if skipped
    
    Raises:
        ValueError: If column not found and allow_missing is False
    """
